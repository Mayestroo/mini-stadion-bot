...
export default function StadiumModerationPage() {
  ...
  const { data: drafts = [], isLoading, isError } = useQuery({ queryKey: ["admin-stadium-drafts"], queryFn: superadminApi.getStadiumDrafts });
  ...
  const reject = useMutation({ mutationFn: (id: number) => superadminApi.rejectStadiumDraft(id, rejectNote || "Rad etildi"), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-stadium-drafts"] }); setRejectingId(null); setRejectNote(""); } });
  ...
  return (
    <AdminShell title="Stadion moderatsiyasi" subtitle="Superadmin tasdig'ini kutayotgan so'rovlar">
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {isLoading ? <AdminLoading /> : isError ? <AdminCard><p style={{ color: "var(--mini-red)", fontWeight: 700 }}>Xatolik yuz berdi. Draftlarni yuklab bo'lmadi.</p></AdminCard> : drafts.length === 0 ? <AdminEmptyState icon={<Warehouse size={28} />} title="So'rovlar yo'q" text="Tasdiq kutayotgan stadion o'zgarishlari shu yerda chiqadi." /> : drafts.map((draft: StadiumDraft) => (
